...
onMounted(() => {
  if (long && lati ) {
    if (userLon && userLat) url.value =`https://www.google.com/maps/embed/v1/directions?key=${GKEY}&origin=${userLat+','+userLon}&destination=${lati+','+long}&zoom=8&avoid=tolls|highways`;
